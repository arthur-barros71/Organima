...
@foreach($erros as $erro)
    ...
    @if($erro->id_usuario == Auth::user()->id)
     ...
     @endif>
        ...
        <div class="errorFrameView" id="errorFrameView_{{ $erro->id }}" onclick="gotoFrame({{ $erro->nr_frame }})"
        @if($efvStyle)
            style="{{ $efvStyle }}"
        @endif>
    ...
    @if($erro->ic_conclusao == 1)
        ...
        <div class="consertoMargin" data-dt="{{ $erro->dt_conclusao }}">
            ...
            <div class="consertoCont">
                <p>{{ $erro->nm_concluidor }} marcou o erro no Frame {{ $erro->nr_frame }} como <span style="color: #8EB7A0">corrigido</span></p>
            </div>
